    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            if (data.success) {
                ...
                setSuccess(data.mock ? '视频生成任务已提交(模拟模式)，正在加载...' : '正在生成视频，请稍候...');
                // 开始轮询（模拟模式也走轮询流程）
            ...
            <Paper elevation={3} sx={{ p: 4, borderRadius: 3 }}>
                        ...
                        {loadingHistory ? (
                            ...
                            <List dense sx={{ maxHeight: 200, overflow: 'auto', bgcolor: 'grey.50', borderRadius: 1 }}>
                                {history.map((item, index) => (
                                    ...
                                    >
                                        <PlayArrowIcon sx={{ fontSize: 20, mr: 1, color: 'secondary.main' }} />
                                        <ListItemText
                                            primary={item.prompt}
                                            secondary={item.params?.knowledgeSource === 'dify' ? '知识库内容' : '原生概念'}
                ...
                {knowledgeSource && (
                    <Box sx={{ mb: 2 }}>
                        <Chip
                            size="small"
                            label={knowledgeSource === 'dify' ? '知识库内容' : '原生概念'}
...
export default KnowledgeVideoGenerator;